...
export const BodySidebar = ({ data }: IProps) => {
  ...
  const Item = ({ data }: Iitem) => {
    const { Icon, SubItem, Tittle, pathname, notification } = data
    return (
      <div className="flex flex-col gap-4 cursor-pointer  rounded-lg p-2  transition-all text-custom1 dark:text-custom5 duration-300 ease-in-out">
        ...
        <div className=" flex flex-col w-full overflow-hidden transition-all gap-2 duration-300 ease-in-out  relative">
          <input
            type="checkbox"
